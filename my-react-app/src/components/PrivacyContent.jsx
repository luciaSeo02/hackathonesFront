import { useEffect, useState } from "react";

const PrivacyContent = ({ lang = "es" }) => {
    const [privacy, setPrivacy] = useState(null);

    useEffect(() => {
        fetch(`/privacy.${lang}.json`)
            .then(res => res.json())
            .then(setPrivacy);
    }, [lang]);

    if (!privacy) return <p>Cargando...</p>;

    return (
        <section className="bg-white -mt-20 pt-10 rounded-3xl gap-3 mb-6
        lg:-mt-20 lg:pt-20 lg:rounded-3xl lg:gap-4 lg:mb-20">
            <div className="max-w-6xl px-8 mx-auto lg:px-0">
                <h3 className="text-2xl font-bold mb-2">{privacy.title}</h3>
                <p className="text-sm mb-4">{privacy.lastUpdate}</p>
                {privacy.sections.map((section, idx) => (
                    <article key={idx} className="mb-4">
                        {section.heading && (
                            <h4 className="font-semibold">{section.heading}</h4>
                        )}
                        <p>{section.content}</p>
                    </article>
                ))}
            </div>
        </section>
    );
};

export default PrivacyContent;